...
module cache_controller #(
    ...
    input  wire                  mode, //mode = read_operation ? 0 : 1 --> 1 if write operation
    input  wire                  req,  //request enable: 1 only when CPU performs a load/store (lw/sw)
    ...
    input  wire             bus_src_id,
    output wire             bus_src_out,
    ...
    assign wait_req = ( mem_rd_pending )|| ( mode == 0 && miss_both )|| (mode==1 && miss_both);
...
always @(posedge clk) begin
    if (rst) begin
        ...
        if (bus_src_id == ~core_id && bus_cmd_in != BUS_IDLE) begin
            // WRITE-INVALIDATE: a remote write (hit or miss) invalidates our copy.
            // Write-through keeps main memory current, so we simply drop the line
            // and re-miss from memory on the next access -- no data is copied off
            // the bus (address-only invalidation).
            if (bus_cmd_in == BUS_UPDATE || bus_cmd_in == BUS_WR) begin
                // L1: invalidate if present
                if (l1_tag[snoop_l1_index] == snoop_l1_tag && l1_msi[snoop_l1_index] != INVALID) begin
                    l1_msi[snoop_l1_index] <= INVALID;
                end
                // L2: invalidate if present - check each way explicitly
                if (L2_WAYS >= 1 && l2_tag[snoop_l2_index][0] == snoop_l2_tag && l2_msi[snoop_l2_index][0] != INVALID) begin
                    l2_msi[snoop_l2_index][0] <= INVALID;
                end
            end
        ...
        if (!mem_rd_pending && req) begin
            if (mode == 0) begin
                ...
                if (l1_hit_detected) begin
                    ...
                    mem_rd_addr    <= addr[BLOCK_OFFSET+5 : BLOCK_OFFSET]; // word index into main memory
                    mem_rd_en      <= 1'b1;
                    mem_rd_pending <= 1'b1;
                ...
                if (l1_hit_detected) begin
                    ...
                    // FIX: keep L2 inclusive/consistent so a later L1 eviction
                    // cannot expose a stale L2 copy of a word this core just wrote
                    l2_data[l2_index][0] <= data_in;
                    l2_tag[l2_index][0]  <= l2_tag_in;
                    l2_msi[l2_index][0]  <= MODIFIED;

                    // Broadcast invalidate (address-only)
                    bus_cmd  <= BUS_WR;
                    bus_addr <= addr;

                    // Write through to memory
                    mem_wr_addr <= addr[BLOCK_OFFSET+5 : BLOCK_OFFSET];
                    mem_wr_data <= data_in;
                    mem_wr_en   <= 1'b1;
                end else if (l2_hit_detected) begin
